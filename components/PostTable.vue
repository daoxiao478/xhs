      ...
      <TableBody>
        <TableRow v-for="(post, index) in posts" :key="post.id" class="hover:bg-gray-50">
          ...
          <TableCell class="cursor-pointer">
            <UserAvatar :src="post.user.avatar" :username="post.user.nickname" :userId="post.user.user_id"
              :xsecToken="post.user.xsec_token" />
          </TableCell>
          <TableCell class="cursor-pointer">
          ...
          <TableCell>
            {{ post.publish_time || '未知' }}
          </TableCell>
          <TableCell>
            <div class="text-left text-sm flex gap-2 cursor-pointer">
              <span>点赞: {{ post.interact_info.liked_count }}</span>
              <span>收藏: {{ post.interact_info.collected_count }}</span>
              <span>评论: {{ post.interact_info.comment_count }}</span>
            </div>
          </TableCell>
          <TableCell class="text-left space-x-2">
            ...
            <Button size="sm" variant="outline" @click="viewPostDetail(post)">
              详情
            </Button>
          </TableCell>
        </TableRow>
